...
With option --jsonoutput, the tool will read all input files and produce JSON output.
...
def ProcessBinaryFile(filename, content, cutexpression, flag, oRules, dResults, oMyJSONOutput, oOutput, oLogfile, options, oParserFlag):
    if content == None:
        try:
    ...
    try:
        ...
        if options.jsonoutput:
            oMyJSONOutput.AddItem(filename, data)
            return
...
def ProcessBinaryFiles(filenames, oLogfile, options, oParserFlag):
    ...
    if options.jsoninput:
        ...
        for item in items:
            ...
            index += 1
            ProcessBinaryFile(item['name'], item['content'], '', '', oRules, dResults, oMyJSONOutput, oOutput, oLogfile, options, oParserFlag)
    else:
        ...
        for filename, cutexpression, flag in filenames:
            ...
            ProcessBinaryFile(filename, None, cutexpression, flag, oRules, dResults, oMyJSONOutput, oOutput, oLogfile, options, oParserFlag)
