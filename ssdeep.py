...
History:
  2021/05/22: start from binary template
...
try:
    ...
    print('Module ppdeep required, use this command to install it: pip install ppdeep')
    exit(-1)
...
def ProcessBinaryFile(filename, content, cutexpression, flag, oOutput, oLogfile, options, oParserFlag):
    ...
    try:
        ...
        ssdeephash = ppdeep.hash(data)
        if options.compare != '':
            comparison = ppdeep.compare(ssdeephash, options.compare)
        if not options.csv:
            if options.compare == '':
                oOutput.Line(ssdeephash)
            else:
                oOutput.Line('%s %d' % (ssdeephash, comparison))
        else:
            row = [filename, ssdeephash]
            if options.compare != '':
...
https://DidierStevens.com'''
    ...
    oParser.add_option('-c', '--compare', type=str, default='', help='ssdeep hash to compare to')
    oParser.add_option('-v', '--csv', action='store_true', default=False, help='CSV output')
